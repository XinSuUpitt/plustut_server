...
// 添加后台用户
let addUser = ( value ) => {
  var _sql = `insert into teachers set username=?,password=?`
  console.log(1)
  return query( _sql, value)
}
// 删除后台用户
let deleteUser = ( name ) => {
  var _sql = `delete from teachers where username="${name}"; `
  return query( _sql)
}
// 查找用户
let findUser = (name) => {
	var _sql = `select * from teachers where username="${name}"; `
  return query( _sql )
}
// 查询所有数据
let findData = (table) => {
...
let insertClass = (value) => {
  var _sql = `insert into classes set class_name=?, elementary=?, teacher_id=?, price=?, start_time=?, end_time=?, start_date=?, end_date=?, week_day=?, description=?;`
  return query(_sql, value)
}
let updateClass = (value) => {
  var _sql = `update classes set class_name=?, elementary=?, teacher_id=?, price=?, start_time=?, end_time=?, start_date=?, end_date=?, week_day=?, description=? where id=?;`
  return query(_sql, value)
}
let deleteClass = (value) => {
  var _sql = `delete from classes where id="${id}";`
  return query(_sql)
}
...
let findMobileUserByName = ( name ) => {
  var _sql = `select * from students where userName="${ name }";`
  return query( _sql )
}

// 添加手机用户
let addMobileUser = ( value ) => {
  var _sql = `insert into students set userName=?,password=?,time=?`
  return query( _sql , value)
}
// 检测用户登录信息的有效性
let checkUser = (value) => {
...
let getTeachersByName = (user) => {
  var _sql = `select * from teachers where username="${user}";`
  return query(_sql)
}
