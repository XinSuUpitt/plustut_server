...
let elementaryclasses = 
    `create table if not exists elementaryclasses(
      id INT NOT NULL AUTO_INCREMENT,
      classname VARCHAR(300) NOT NULL,
      teachername VARCHAR(100) NOT NULL,
      teacheravator VARCHAR(100) NOT NULL DEFAULT '',
      price INT NOT NULL DEFAULT 100,
      description VARCHAR(1000) NOT NULL,
      PRIMARY KEY ( id ) 
    );`
let middleclasses = 
    `create table if not exists middleclasses(
      id INT NOT NULL AUTO_INCREMENT,
      classname VARCHAR(300) NOT NULL,
      teachername VARCHAR(100) NOT NULL,
      teacheravator VARCHAR(100) NOT NULL DEFAULT '',
...
createTable(elementaryclasses)
createTable(middleclasses)
createTable(users)
...
// 删除后台用户
let deleteUser = ( name ) => {
  var _sql = `delete from users where username="${name}"; `
  return query( _sql)
}
// 查找用户
let findUser = (name) => {
...
//elementary class
let insertElementaryClass = (value) => {
  var _sql = `insert into elementaryclasses set classname=?, teachername=?, teacheravator=?, price=?, description=?;`
  return query(_sql, value)
}
let updateElementaryClass = (value) => {
  var _sql = `update elementaryclasses set classname=?, teachername=?, teacheravator=?, price=?, description=? where id=?;`
  return query(_sql, value)
}
let deleteElementaryClass = (value) => {
  var _sql = `delete from elementaryclasses where id="${id}";`
  return query(_sql)
}
let getElementaryDataById = ( id ) => {
  var _sql = `select * from elementaryclasses where id="${id}"; `
  return query( _sql )
}

//middle class
let insertMiddleClass = (value) => {
  var _sql = `insert into middleclasses set classname=?, teachername=?, teacheravator=?, price=?, description=?;`
  return query(_sql, value)
}
let updateMiddleClass = (value) => {
  var _sql = `update middleclasses set classname=?, teachername=?, teacheravator=?, price=?, description=? where id=?;`
  return query(_sql, value)
}
let deleteMiddleClass = (value) => {
  var _sql = `delete from middleclasses where id="${id}";`
  return query(_sql)
}
let getMiddleDataById = ( id ) => {
  var _sql = `select * from middleclasses where id="${id}"; `
  return query( _sql )
}
...
let getelementaryclasses = ( value ) => {
  var _sql = `select * from elementaryclasses`
  return query(_sql)
} 
let getmiddleclasses = ( value ) => {
  var _sql = `select * from middleclasses`
  return query(_sql)
} 
...
module.exports = {
  ...
  findDataByCls,
  getDataById,
  insertElementaryClass,
  insertMiddleClass,
  updateElementaryClass,
  updateMiddleClass,
  deleteElementaryClass,
  deleteMiddleClass,
  getelementaryclasses,
  getmiddleclasses,
