...
// 存储手机端的用户信息
router.post('/api/signin', koaBody(), async (ctx, next) => {
...
// 获取单个id的信息
router.post('/api/getVideoById', koaBody() ,async (ctx) => {
...
// 获取文章的评论
router.post('/api/getVideoComment', koaBody(), async (ctx) => {
...
// 获取用户的评论
router.post('/api/getUserComment', koaBody(),async (ctx) => {
...
// 评论
router.post('/api/postComment', koaBody(),async(ctx) => {
...
// 删除评论
router.post('/api/deleteComment', koaBody(),async(ctx) => {
    await checkToken(ctx).then(async res => {
        // console.log(res)
...
// 点击喜欢
router.post('/api/postUserLike', koaBody(),async(ctx) => {
...
// 获取单个video的like信息
router.post('/api/getUserSingleLike', koaBody() ,async (ctx) => {
...
// 获取个人like列表
router.post('/api/getUserLikeData', koaBody(), async (ctx) => {
...
// 修改用户名
router.post('/api/editUserName', koaBody(), async(ctx,next)=>{
...
// 获取用户头像
router.post('/api/getUserAvator',koaBody(),async(ctx)=>{
...
// 增加头像
router.post('/api/uploadAvator',koaBody({
    "formLimit":"5mb",
    "jsonLimit":"5mb",
    "textLimit":"5mb"
}),async(ctx)=>{
...
// 验证码
router.get('/api/getYzm',async(ctx,next)=>{
...
router.get('/api/getteachers', koaBody(), async(ctx)=>{
    var val = ctx.request.body.val
    // console.log(val)
    await apiModel.getteachers(val).then(res=>{
        // console.log('搜索结果',res)
        ctx.body = {
            code:200,
            data:res,
...
router.get('/api/get_articles', koaBody(), async(ctx) => {
    var val = ctx.request.body.val
    // console.log(val)
    await apiModel.getarticles(val).then(res=>{
        // console.log('搜索结果',res)
        ctx.body = {
            code:200,
            data:res,
